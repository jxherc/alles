  ...
  try {
    ...
    list.innerHTML = entries.map(e => `
      <div class="vault-entry" data-id="${e.id}">
        <span class="vault-entry-name">${_esc(e.name)}</span>
...
async function _addEntry() {
  ...
  const cat  = document.getElementById('vault-new-cat')?.value.trim() || 'general';
  if (!name || !val) { toast('name and value required', 'error'); return; }
  const r = await fetch('/api/vault', {
    method: 'POST',
    headers: { 'content-type': 'application/json' },
    body: JSON.stringify({ name, value: val, category: cat }),
  });
  if (!r.ok) { toast('failed — is vault unlocked?', 'error'); return; }
  ['vault-new-name','vault-new-value','vault-new-cat'].forEach(id => {
