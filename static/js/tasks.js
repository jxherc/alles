...
function _rowHtml(t, child, progress) {
  return `
    <div class="task-item${child ? ' task-child' : ''}" data-id="${t.id}">
      ...
      ${!child ? `<button class="task-addsub" data-id="${t.id}" title="${esc(tr('tasks.add_subtask'))}">${esc(tr('tasks.sub'))}</button>` : ''}
...
function openTaskEditor(id) {
  ...
  ov.innerHTML = `<div class="task-editor">
    ...
    <textarea class="settings-input te-notes" id="te-notes" placeholder="${esc(tr('tasks.notes'))}">${esc(t.notes || '')}</textarea>
    <div class="te-row"><label>${esc(tr('tasks.priority.label'))}</label><div class="settings-input custom-select" id="te-prio"></div></div>
    <div class="te-row"><label>${esc(tr('tasks.repeat.label'))}</label><div class="settings-input custom-select" id="te-rep"></div></div>
    <div class="te-row"><label>${esc(tr('tasks.due'))}</label><input class="settings-input" id="te-due" value="${esc((t.due_date || '').slice(0, 10))}" placeholder="YYYY-MM-DD"></div>
    ...
    <div class="te-row"><label>${esc(tr('tasks.tags'))}</label><input class="settings-input" id="te-tags" value="${esc((t.tags || []).join(', '))}" placeholder="${esc(tr('tasks.tags_placeholder'))}"></div>
    <div class="te-row"><label>${esc(tr('tasks.project'))}</label><input class="settings-input" id="te-proj" value="${esc(t.project || '')}"></div>
    <div class="te-actions"><button class="btn" id="te-cancel">${esc(tr('common.cancel'))}</button><button class="btn primary" id="te-save">${esc(tr('common.save'))}</button></div>
  </div>`;
