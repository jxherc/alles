...
class MoneyCsvTest(ApiTest):
    def _account(self):
        d = self.db()
        a = Account(name="checking", opening=0.0)
        d.add(a)
        d.commit()
        aid = a.id
        d.close()
    ...
    def test_export_then_import_roundtrip(self):
        ...
        self.client.post(
            "/api/money/transactions",
            json={"account_id": aid, "date": "2026-06-02", "amount": 100.0, "category": "income"},
        )
        ...
        aid2 = self._account()
        r = self.client.post(
            "/api/money/import.csv".replace("import.csv", "transactions/import.csv"),
    ...
    def test_import_skips_duplicates(self):
        ...
        second = self.client.post(
            "/api/money/transactions/import.csv", json={"csv": again, "account_id": aid}
        ).json()
        ...
        third = self.client.post(
            "/api/money/transactions/import.csv", json={"csv": dupfile, "account_id": aid2}
        ).json()
