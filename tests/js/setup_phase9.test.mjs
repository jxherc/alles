...
assert.match(wizard, /import \{ wireChoiceGroup \} from '\.\/kokuen\.js\?v=1'/);
assert.match(wizard, /wireChoiceGroup\(group\)/);
assert.match(wizard, /button\.setAttribute\('aria-busy', 'true'\)/);
assert.match(wizard, /button\.removeAttribute\('aria-busy'\)/);
assert.match(wizard, /button\?\.getAttribute\('aria-busy'\) === 'true'/);
...
assert.match(wizard, /async function _dismiss\(\) \{[\s\S]{0,240}_loadSequence \+= 1/);
...
assert.doesNotMatch(wizard, /function _bindChoiceGroup[\s\S]{0,900}Arrow(?:Left|Right|Up|Down)/);
