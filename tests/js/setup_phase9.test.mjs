...
assert.match(wizard, /let _loadSequence = 0/);
...
assert.match(wizard, /async function _dismiss\(\) \{\s*_loadSequence \+= 1/);
assert.match(wizard, /if \(!resume && \(_dismissedThisSession \|\| status\?\.setup\?\.dismissed \|\| status\?\.setup\?\.completed\)\) return/);
assert.match(wizard, /_dismissedThisSession = true;\s*_close\(\)/);
