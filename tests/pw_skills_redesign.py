...
def main():
    ...
    with sync_playwright() as p:
        ...
        first_pinned = pg.eval_on_selector(
        ...
        if not first_pinned:
            ...
            r["pin_adds_rail_group"] = pg.eval_on_selector(
                ".skl-rail-cat[data-cat='pinned']", "el => !!el"
            )
            r["pin_floats_top"] = pg.eval_on_selector(
                "#skl-grid .skl-card:first-child .skl-pin", "el => el.classList.contains('on')"
            )
            pg.eval_on_selector(
                "#skl-grid .skl-card:first-child .skl-pin", "el => el.click()"
            )  # restore
        ...
        r["drawer_esc_closes"] = pg.eval_on_selector(
            "#skl-drawer", "el => !el.classList.contains('open')"
        )
        ...
        r["mobile_cards_render"] = (
            pg.eval_on_selector_all("#skl-grid .skl-card", "els => els.length") > 0
        )
