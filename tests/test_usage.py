...
class UsageTests(unittest.TestCase):
    def test_aggregation(self):
        db = _mkdb()
        _msg(db, "assistant", {"usage": {"prompt_tokens": 100, "completion_tokens": 50}, "model": "claude-opus-4-8"}, datetime(2026, 6, 14))
        _msg(db, "assistant", {"usage": {"prompt_tokens": 200, "completion_tokens": 80}, "model": "claude-opus-4-8"}, datetime(2026, 6, 20))
        _msg(db, "assistant", {"usage": {"input_tokens": 10, "output_tokens": 5}, "model": "deepseek-v4"}, datetime(2026, 5, 1))  # anthropic-style keys
