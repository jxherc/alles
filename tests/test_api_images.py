...
class ImagesApiTest(ApiTest):
    ...
    def test_image_model_detection(self):
        for mid in ["dall-e-3", "gpt-image-1", "imagen-3.0-generate-002", "flux.1-dev",
                    "stable-diffusion-xl", "sdxl-turbo", "ideogram-v2", "recraft-v3"]:
    ...
    def test_endpoint_exposes_image_models(self):
        d = self.db()
        d.add(ModelEndpoint(name="OpenAI", base_url="http://x", enabled=True,
    ...
    def test_chat_image_no_endpoint_400(self):
        ...
        d = self.db(); s = Sess(name="t"); d.add(s); d.commit(); sid = s.id; d.close()
    ...
    def test_chat_image_full_flow(self):
        ...
        buf = BytesIO(); Image.new("RGB", (4, 4), (200, 30, 30)).save(buf, "PNG"); png = buf.getvalue()
        d = self.db()
        ep = ModelEndpoint(name="OpenAI", base_url="http://x", enabled=True,
                           cached_models=json.dumps(["gpt-4o"]), image_models=json.dumps(["dall-e-3"]))
        s = Sess(name="new chat")
        d.add(ep); d.add(s); d.commit(); sid, epid = s.id, ep.id; d.close()
        ...
        async def fake_gen(*a, **k): return [png]
        ig.generate = fake_gen
        try:
            r = self.client.post("/api/images/chat", json={
                "session_id": sid, "prompt": "a red square", "model": "dall-e-3", "endpoint_id": epid})
        finally:
    ...
    def test_chat_image_incognito_leaves_no_trace(self):
        ...
        buf = BytesIO(); Image.new("RGB", (4, 4), (20, 200, 30)).save(buf, "PNG"); png = buf.getvalue()
        d = self.db()
        ep = ModelEndpoint(name="OpenAI", base_url="http://x", enabled=True,
        ...
        try:
            r = self.client.post("/api/images/chat", json={
                "session_id": sid, "prompt": "secret", "model": "dall-e-3", "endpoint_id": epid})
        finally:
