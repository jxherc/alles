...
def _msgs():
    return [
        {"uid": "1", "from": "alice@x.com", "subject": "Lunch plans", "date": "d1", "date_ts": 100, "seen": True},
        {"uid": "2", "from": "bob@y.com", "subject": "Invoice #42", "date": "d2", "date_ts": 300, "seen": False},
        {"uid": "3", "from": "carol@z.com", "subject": "re: Lunch", "date": "d3", "date_ts": 200, "seen": False},
    ]
...
class MailCacheTest(ApiTest):
    def _account(self):
        ...
        d.add(a); d.commit(); aid = a.id; d.close()
        return aid
    ...
    def test_cached_route_instant(self):
        aid = self._account()
        d = self.db(); mail_cache.save(d, aid, "INBOX", _msgs()); d.close()
