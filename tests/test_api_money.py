...
class MoneyApiTest(ApiTest):
    def test_account_balance_reflects_transactions(self):
        a = self.client.post("/api/money/accounts", json={"name": "checking", "opening": 100.0}).json()
        self.assertEqual(a["balance"], 100.0)
        aid = a["id"]
        self.client.post("/api/money/transactions", json={"account_id": aid, "date": "2026-06-01", "amount": 50.0})
        self.client.post("/api/money/transactions", json={"account_id": aid, "date": "2026-06-02", "amount": -30.0})
    ...
    def test_txn_unknown_account_400(self):
        r = self.client.post("/api/money/transactions", json={"account_id": "nope", "date": "2026-06-01", "amount": 5})
