...
def main():
    with sync_playwright() as p:
        b = p.chromium.launch()
        pg = b.new_context(
            service_workers="block", viewport={"width": 1300, "height": 900}
        ).new_page()
        pg.on(
            "console",
            lambda m: (
                errs.append(m.text)
                if m.type == "error" and "favicon" not in m.text and "net::" not in m.text
                else None
            ),
        )
        ...
        pg.evaluate(
            "()=>{localStorage.removeItem('alles-appearance');localStorage.removeItem('aide-accent');localStorage.removeItem('aide-theme');}"
        )
        pg.reload(wait_until="domcontentloaded")
        pg.wait_for_timeout(700)

        # ── #35: open the unified settings home on the themes pane ──
        pg.evaluate("()=>window._openSettings('themes', true)")
        pg.wait_for_timeout(500)
        navs = pg.eval_on_selector_all(
            "#settings-modal .s-nav-item",
        ...
        check("unified_settings_keeps_core_sections", expected_sections.issubset(navs))

        # ── #33: inline editor present, no 'open editor' button ──
        check(
            "inline_editor_present",
            pg.query_selector("#theme-editor-inline .te-presets") is not None,
        )
        check("no_open_editor_button", pg.query_selector("#s-open-theme-editor") is None)
        check(
            "default_tile_present",
            pg.query_selector('.te-preset[data-preset="default"]') is not None,
        )
        check(
            "no_dark_light_tiles",
            pg.query_selector('.te-preset[data-preset="dark"]') is None
            and pg.query_selector('.te-preset[data-preset="light"]') is None,
        )
        ...
        pg.wait_for_timeout(500)
        check(
            "preset_locks_controls",
            pg.eval_on_selector("#s-default-theme", "el=>el.classList.contains('locked')"),
        )
        check(
            "lock_note_visible",
            pg.eval_on_selector(
                "#s-theme-lock-note",
                "el=>el.offsetParent!==null && el.textContent.includes('sakura')",
            ),
        )
        check(
            "sakura_bg_on", pg.evaluate("()=>document.body.classList.contains('bg-pattern-petals')")
        )
        check(
            "sakura_is_light", pg.evaluate("()=>document.documentElement.dataset.theme==='light'")
        )
        ...
        check(
            "default_bg_none",
            pg.evaluate("()=>![...document.body.classList].some(c=>c.startsWith('bg-pattern-'))"),
        )
        check(
            "default_is_dark", pg.evaluate("()=>document.documentElement.dataset.theme!=='light'")
        )
        ...
        pg.eval_on_selector(
            '#s-accent-swatches .accent-swatch[data-hex="#a78bfa"]', "el=>el.click()"
        )
