...
class PersonasApiTest(ApiTest):
    ...
    def test_patch_and_delete(self):
        ...
        r = self.client.patch(
            f"/api/personas/{pid}", json={"name": "p2", "system_prompt": "be terse"}
        )
    ...
    def test_missing_404(self):
        self.assertEqual(
            self.client.patch("/api/personas/nope", json={"name": "x"}).status_code, 404
        )
    ...
    def test_partial_patch_keeps_other_fields(self):
        ...
        pid = self.client.post(
            "/api/personas", json={"name": "coder", "model": "gpt-x", "is_default": True}
        ).json()["id"]
    ...
    def test_temperature_roundtrip(self):
        pid = self.client.post("/api/personas", json={"name": "loose", "temperature": 1.3}).json()[
            "id"
        ]
        ...
        self.assertIsNone(
            [p for p in self.client.get("/api/personas").json() if p["id"] == pid2][0][
                "temperature"
            ]
        )
    ...
    def test_initial_message_roundtrip(self):
        ...
        p = self.client.post(
            "/api/personas", json={"name": "rev", "initial_message": "review this:"}
        ).json()
        self.assertEqual(p["initial_message"], "review this:")
        r = self.client.patch(
            f"/api/personas/{p['id']}", json={"initial_message": "explain this:"}
        ).json()
    ...
    def test_persona_pins_model_and_switches_endpoint(self):
        ...
        db.add_all([ep1, ep2])
        db.commit()
        p = Persona(name="coder", model="m2")
        db.add(p)
        db.commit()
        s = Session(name="t", persona_id=p.id, endpoint_id=ep1.id)
        db.add(s)
        db.commit()
