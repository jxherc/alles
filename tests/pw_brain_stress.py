...
INSIGHTS = [
    {
        ...
        "title": XSS,
        "body": XSS2,
        "evidence": ['<svg onload="window.__xss=1">', "ok:tag"],
        "pinned": False,
    },
    {
        "id": "x2",
        "title": "normal title with a SENTINEL word",
        "body": HUGE,
        "evidence": [],
        "pinned": True,
    },
] + [
    {"id": f"v{i}", "title": f"vol insight {i}", "body": "b", "evidence": ["e"], "pinned": False}
    for i in range(500)
]
...
DISTILLED = [
    ...
    {
        "id": "d5",
        "text": "normal SENTINEL2 fact",
        "confidence": 0.82,
        "provenance": "sessions:9",
        "vetoed": False,
        "pinned": False,
    },
] + [
    {"id": f"q{i}", "text": f"vol fact {i}", "confidence": 0.6, "vetoed": False, "pinned": False}
    for i in range(500)
]
...
def main():
    ...
    with sync_playwright() as p:
        ...
        assert pg.evaluate(
            "() => document.querySelectorAll('#brain-insights img, #brain-insights svg, #brain-insights script').length === 0"
        ), "raw HTML node injected from insight payload"
        assert pg.evaluate(
            "() => document.querySelectorAll('#brain-usermodel img, #brain-usermodel script').length === 0"
        ), "raw HTML node injected from distilled payload"
        print("PASS xss: no execution, payloads rendered inert")

        # 2. confidence bar clamped 0..100 (no NaN / overflow widths)
        widths = pg.evaluate(
            """() => [...document.querySelectorAll('#brain-usermodel .um-conf i')].map(i => i.style.width)"""
        )
        bad = [
            w
            for w in widths
            if w
            and (w == "NaN%" or (w.endswith("%") and (float(w[:-1]) < 0 or float(w[:-1]) > 100)))
        ]
        ...
        n_ins = pg.evaluate(
            "() => document.querySelectorAll('#brain-insights .brain-insight').length"
        )
        ...
        print(
            f"PASS volume: {n_ins} insights + {n_um} facts rendered, no overflow (body {body_w} <= win {win_w})"
        )
