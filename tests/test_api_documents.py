...
class DocumentsApiTest(ApiTest):
    def test_crud(self):
        ...
        r = self.client.patch(f"/api/documents/{did}", json={"content": "longer text", "title": "t2"})
    ...
    def test_missing_404(self):
        ...
        self.assertEqual(self.client.patch("/api/documents/nope", json={"title": "x"}).status_code, 404)
    ...
    def test_ai_edit_without_endpoint_400(self):
        ...
        r = self.client.post(f"/api/documents/{did}/ai-edit", json={"instruction": "make it better"})
        self.assertEqual(r.status_code, 400)   # no endpoint configured

    def test_ai_edit_missing_doc_404(self):
        self.assertEqual(self.client.post("/api/documents/nope/ai-edit", json={"instruction": "x"}).status_code, 404)
