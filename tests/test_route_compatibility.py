...
class RouteCompatibilityBaselineTest(unittest.TestCase):
    def test_full_method_path_surface_matches_phase_two_snapshot(self):
        rows = _route_rows()
        digest = hashlib.sha256(("\n".join(rows) + "\n").encode()).hexdigest()
        self.assertEqual(len(rows), 715)
        self.assertEqual(digest, "5e38485675c05c8711fdea9a9a75f0b9ff9092051b5cd64695444f73c2f1a658")
        groups = Counter(
            "api"
        ...
        self.assertEqual(groups, {"api": 698, "v1": 2, "public": 15})
