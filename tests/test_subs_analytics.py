...
class AutoPostTests(unittest.TestCase):
    def _setup(self, days_ago=3, cycle="monthly", account=True):
        ...
        sub = _sub(name="Netflix", price=15, cycle=cycle,
                   account_id=acct.id if account else "", next_due=due)
    ...
    def test_posts_charge_to_linked_account(self):
        ...
        changed = S._roll_and_post(sub, date.today(), db); db.commit()
    ...
    def test_idempotent_second_roll_no_double_post(self):
        ...
        from core.database import Transaction
        db, sub, acct, due = self._setup()
        S._roll_and_post(sub, date.today(), db); db.commit()
        again = S._roll_and_post(sub, date.today(), db); db.commit()
