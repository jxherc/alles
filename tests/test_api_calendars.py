...
class CalendarsApiTest(ApiTest):
    ...
    def test_create_event_gets_default_calendar(self):
        ev = self.client.post(
            "/api/calendar", json={"title": "x", "start_dt": "2026-06-01T10:00"}
        ).json()
    ...
    def test_delete_scope_this_adds_exception(self):
        ev = self.client.post(
            "/api/calendar",
            json={"title": "daily", "start_dt": "2026-06-01T09:00", "recurrence": "daily"},
        ).json()
    ...
    def test_delete_all_removes(self):
        ev = self.client.post(
            "/api/calendar", json={"title": "z", "start_dt": "2026-06-01T09:00"}
        ).json()
    ...
    def test_delete_calendar_moves_events_to_default(self):
        ...
        ev = self.client.post(
            "/api/calendar",
            json={"title": "w", "start_dt": "2026-06-01T09:00", "calendar_id": cal["id"]},
        ).json()
