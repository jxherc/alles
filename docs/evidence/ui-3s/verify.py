...
def run():
    ...
    with sync_playwright() as p:
        ...
        if pop:
            ...
            ok(
                "explainer says what it does",
                "action item" in txt or "to-do" in txt or "tasks" in txt,
            )
        ...
        bl = pg.evaluate(
            "() => (document.querySelector('.wiki-bl-explainer')?.textContent || document.querySelector('#wiki-backlinks')?.textContent || '')"
        )
        ok(
            "backlinks explains itself",
            "backlink" in bl.lower() and ("unlinked" in bl.lower() or "link" in bl.lower()),
        )
