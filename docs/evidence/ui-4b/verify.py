...
def run():
    ...
    with sync_playwright() as p:
        ...
        ok(
            "rows are labelled",
            "inbox" in d["labels"] and "sent" in d["labels"] and "drafts" in d["labels"],
        )
        ...
        ok(
            ...
            pg.evaluate("() => document.querySelector('.mail-nav-item.active')?.dataset.filter")
            == "sent",
        )
        ...
        collapsed = pg.evaluate(
            "() => document.querySelector('#mail-view').classList.contains('sidebar-collapsed')"
        )
        ok("toggle collapses the sidebar", collapsed)
        ok(
            "collapsed state is persisted",
            pg.evaluate("() => localStorage.getItem('mail-sidebar-collapsed')") == "1",
        )
        labelHidden = pg.evaluate(
            "() => getComputedStyle(document.querySelector('.mail-nav-label')).display === 'none'"
        )
