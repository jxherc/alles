...
| `s3` | external S3-compatible bucket | Backup-only destination. Its remote contents are not copied into an archive; the manifest records `configured` or `not-configured`. |
...
| Settings and connector config | `settings.json`, `caldav.json`, `carddav.json`, `webdav_backup.json`, `s3_backup.json`, and SQLite rows | Included when present. New backups require every known credential to be sealed and authenticated with its exact field purpose. WebDAV uses `backup.webdav.password`; the S3-compatible access-key pair uses `backup.s3.credentials`. Historical plaintext archives remain migration-compatible. |
...
- full remote mailboxes, calendars, contact books, object stores, model providers, MCP servers, Discord,
  Telegram, webhook targets, and the WebDAV/S3-compatible backup stores: Alles backs up its local
  state, not the remote system's authoritative copy. A backup store receives only an encrypted backup
  artifact.
...
- The S3-compatible config is frozen and purpose-bound in the same snapshot. A synthetic signed-S3
  recovery test deletes the source install, reconnects with a separately held access-key pair, downloads
  the remote encrypted artifact, and restores every credential with the separately saved recovery key.
