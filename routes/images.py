...
async def generate_in_chat(body: ChatImageBody, db: DbSession = Depends(get_db)):
    ...
    for i, raw in enumerate(imgs):
        ...
        saved.append(Photo(filename=info["filename"], thumb=info["thumb"],
                           original_name=(body.prompt[:60] or "generated") + ".png",
                           width=info["width"], height=info["height"],
                           taken_at=info["taken_at"], exif=info["exif"]))
    if not saved:
    ...
    doc = Document(title=title, doc_type="md",
    ...
    db.add(Message(session_id=s.id, role="assistant", content=assistant_md,
    ...
    return {"content": assistant_md, "doc_id": doc.id, "doc_title": title,
