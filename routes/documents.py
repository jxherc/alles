...
def patch_doc(doc_id: str, body: PatchDoc, db: DbSession = Depends(get_db)):
    ...
    if body.content is not None:  d.content = body.content
    if body.doc_type is not None: d.doc_type = body.doc_type
...
async def ai_edit(doc_id: str, body: AiEditRequest, db: DbSession = Depends(get_db)):
    ...
    if not ep: raise HTTPException(400, "no endpoint configured")
    model = ep.models_list()[0] if ep.models_list() else ""
    if not model: raise HTTPException(400, "no model available")

    from services.llm import stream_chat
    msgs = [
        {"role": "system", "content": "You are a document editor. Rewrite the document per the instruction. Return ONLY the new document content — no commentary, no markdown fences."},
