...
async def _fire(db, rule, ctx: dict):
    ...
    text = _render(rule.action_arg, ctx) or ctx.get("subject") or ctx.get("name") or rule.name or "automation"
    try:
        if rule.action == "create_task":
            ...
            await broadcast({"title": rule.name or "alles", "body": text[:300], "url": "/",
                             "tag": f"auto-{rule.id}-{ctx.get('dedupe', '')}"})
        elif rule.action == "push_digest":
            from routes.push import broadcast
            body = _digest(db)
            await broadcast({"title": "your day", "body": body, "url": "/", "tag": f"digest-{rule.id}"})
...
def _digest(db) -> str:
    ...
    if d["events"]:
        first = d["events"][0]
        bits.append(f"{len(d['events'])} event{'s' if len(d['events']) != 1 else ''} (first: {first['time'] or 'all-day'} {first['title']})")
    od, dt_ = len(d["tasks"]["overdue"]), len(d["tasks"]["due_today"])
    if od: bits.append(f"{od} overdue task{'s' if od != 1 else ''}")
    if dt_: bits.append(f"{dt_} due today")
    if d["renewing"]: bits.append(f"{len(d['renewing'])} renewal{'s' if len(d['renewing']) != 1 else ''} this week")
            ...
            try:
                ...
                if rule.trigger == "daily_at":
                    ...
                    if now.strftime("%H:%M") >= hhmm and st.get("last_daily") != today.isoformat():
                        ...
                        rule.state = json.dumps(st); db.commit()
                        await _fire(db, rule, {"date": today.isoformat(), "dedupe": today.isoformat()})
                    ...
                    for s in db.query(Subscription).filter(Subscription.active == True).all():
                        ...
                        if 0 <= (sub_parse(s.next_due) - today).days <= days:
                            ...
                            st["done"] = _trim(done)
                            rule.state = json.dumps(st); db.commit()
                            await _fire(db, rule, {"name": s.name, "date": s.next_due,
                                                   "price": s.price, "dedupe": key})
                    ...
                    for ev in db.query(DayEvent).all():
                        ...
                        if 0 <= (target - today).days <= days:
                            ...
                            st["done"] = _trim(done)
                            rule.state = json.dumps(st); db.commit()
                            await _fire(db, rule, {"name": ev.name, "date": target.isoformat(), "dedupe": key})
...
async def _check_mail_rule(db, rule, st):
    ...
    for a in db.query(MailAccount).all():
        acct = {"imap_host": a.imap_host, "imap_port": a.imap_port,
        ...
        for m in msgs:
            ...
            if needle not in (m.get("from", "") or "").lower():
                continue
            await _fire(db, rule, {"from": m.get("from", ""), "subject": m.get("subject", ""),
                                   "date": m.get("date", ""), "dedupe": f"{a.id}:{m.get('uid')}"})
    st["uids"] = seen_uids
...
async def on_doc_saved(path: str, content: str):
    ...
    try:
        rules = db.query(AutomationRule).filter(
            AutomationRule.enabled == True, AutomationRule.trigger == "doc_tag").all()
        for rule in rules:
            ...
            st["done"] = _trim(done)
            rule.state = json.dumps(st); db.commit()
            await _fire(db, rule, {"path": path, "tag": tag, "name": path.rsplit("/", 1)[-1], "dedupe": path})
    except Exception as e:
